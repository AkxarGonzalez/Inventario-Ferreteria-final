            ...
            if (e.KeyChar == (char)Keys.Enter)
            {
                ...
                if ((txtCantidadTotal.Text != "" && txtCodigodeBarras.Text != "") || txtCodigodeBarras.Text != "0")
                {
                    ...
                    using (var conexion = new SqlConnection(strConexion))
                    {
                        ...
                        // por cadena de texto
                       // var cmd = new SqlCommand("UPDATE inventario set Movimiento='"+invnvo.Movimiento+"', Fecha='"+invnvo.Fecha+"' ,cantidad=" + invnvo.Cantidad + " WHERE codigo='" + invnvo.Codigo + "'", conexion);
